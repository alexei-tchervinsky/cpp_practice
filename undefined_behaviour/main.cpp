...
/*
   ...
   Перегрузка операторов является вызовом функции, а для функций порядок вычмсления аргументов неспецифицирован (unspecified behaviour) - поэтому при перегрузке операторов &&, || и , - невозможно определить порядок вычислений - нарушается последовательность точек вызова
